(function () {
	    ...
	    [].forEach.call(countBoxEls, function (el) {
	      ...
	      var maxHandler = function maxHandler(value) {
	        if (value) {
			  ...
			  $('.pd-side-count__note p.error').text("已達購買上限");
	        } else {
	          btnPlus.disabled = false;
			  $('.pd-side-count__note p.error').text("");
	        }
	      };
	...
	//判斷商品list是否為已售完狀態，已售完 hover 無樣式 
	window.addEventListener('DOMContentLoaded', function() {
		var liItems = document.querySelectorAll('.pd-card__item');

		liItems.forEach(function(li) {
			var imgMask = li.querySelector('.pd-card__img-mask');
			if (imgMask) {
